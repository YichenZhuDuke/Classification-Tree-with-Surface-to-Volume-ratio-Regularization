...
def experiment_runner(seed, Xall, Yall, n1, n0, pen_lst, weight, c0, alpha_lst):
    ...
    num_feats = np.zeros((max(len(alpha_lst),len(pen_lst)), 6))       
    '''SVR tree'''
    for j in range(len(pen_lst)):
        ...
        num_feats[j,0] +=sum(tr_svr.feats_usage)        
    ...
    for j in range(len(alpha_lst)):
        ...
        num_feats[j,2] +=sum(treelst[i].feats_usage)        
    '''CART with SMOTE'''
    ...
    for j in range(len(alpha_lst)):
        ...
        treelst[i].compute_feats_usage()
        num_feats[j,3] +=sum(treelst[i].feats_usage)        
    '''CART with B-SMOTE'''
    ...
    for j in range(len(alpha_lst)):
        ...
        treelst[i].compute_feats_usage()
        num_feats[j,4] +=sum(treelst[i].feats_usage)        
    '''CART with ADASYN'''
    ...
    for j in range(len(alpha_lst)):
        ...
        treelst[i].compute_feats_usage()
        num_feats[j,5] +=sum(treelst[i].feats_usage)    
    return (Trate_svr, Trate_svr_feat_select, Trate_duplicate, Trate_SMOTE, Trate_BSMOTE, Trate_ADASYN, num_feats)
...
num_feats = np.zeros((max(len(alpha_lst),len(pen_lst)), 6))   ## records the number of featured used for: svr, svr_feat_select, duplicate, SMOTE, BSMOTE, ADASYN
for i in range(nexps):
    ...
    num_feats = num_feats + output_now[6]
...
num_feats_select = np.zeros(6)
for k in range(6):
    num_feats_select[k] = num_feats[select_ind[k], k]
